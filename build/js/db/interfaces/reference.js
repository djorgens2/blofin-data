...
async function Add(table, data) {
    const [fields, args] = (0, query_utils_1.parseColumns)(data, "");
    if (fields) {
        args[0] === 0 && (args[0] = (0, crypto_util_1.hashKey)(6));
...
//+--------------------------------------------------------------------------------------+
async function Fetch(table, props) {
    const [fields, args] = (0, query_utils_1.parseColumns)(props);
    const sql = `SELECT * FROM blofin.${table} WHERE ${fields.join(" AND ")}`;
