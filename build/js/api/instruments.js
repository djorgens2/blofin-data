...
function Publish(Instruments) {
    ...
    Instruments.forEach(async (instrument) => {
        const symbol = instrument.instId.split("-");
        if (symbol[0] === instrument.baseCurrency && symbol[1] === instrument.quoteCurrency) {
            await currency.Publish(instrument.baseCurrency, instrument.state !== 'live');
            await currency.Publish(instrument.quoteCurrency, false);
        }
    });
}
;
