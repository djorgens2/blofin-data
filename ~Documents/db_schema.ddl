...
CREATE INDEX ie_sr_stop_request_update_time ON devel.stop_request ( stop_request, update_time );
...
CREATE VIEW devel.vw_api_stop_requests AS with Instrument as (
select
...
select
	...
	row_number() OVER (PARTITION BY sr.stop_request
ORDER BY
	sr.update_time desc ) AS rownum
from
...
select
	...
	max(so.client_order_id) AS clientOrderId,
	max((case when (st.source_ref = 'tp') then replace(format(sr.trigger_price, i.digits, 'en_US'), ',', '') end)) AS tpTriggerPrice,
	max((case when (st.source_ref = 'tp') then replace(format(sr.order_price, i.digits, 'en_US'), ',', '') end)) AS tpOrderPrice,
	max((case when (st.source_ref = 'sl') then replace(format(sr.trigger_price, i.digits, 'en_US'), ',', '') end)) AS slTriggerPrice,
	max((case when (st.source_ref = 'sl') then replace(format(sr.order_price, i.digits, 'en_US'), ',', '') end)) AS slOrderPrice
from
	((((devel.stop_request sr
join devel.instrument_position ipos on
...
join devel.stop_type st on
	((st.stop_type = sr.stop_type)))
left join devel.stop_order so on
	(((coalesce(so.client_order_id, so.tpsl_id) = sr.stop_request) and (so.stop_type = sr.stop_type))))
group by
...
select
	ipos.account AS account,
	conv(hex(op.tpsl_id), 16, 10) AS tpslId,
	s.status AS status,
	i.instId AS instId,
	ipos.position AS positionSide,
	sc.margin_mode AS marginMode,
	sc.action AS side,
	sc.size AS size,
	...
	if((sc.reduce_only = 0), 'false', 'true') AS reduceOnly,
	op.clientOrderId AS clientOrderId,
	sc.broker_id AS brokerId,
	sc.memo AS memo,
	sc.update_time AS update_time
from
	((((OrderPrice op
join devel.instrument_position ipos on
	((ipos.instrument_position = op.instrument_position)))
join StopsCommon sc on
	(((sc.stop_request = op.stop_request) and (sc.rownum = 1))))
join Instrument i on
	((i.instrument = ipos.instrument)))
join devel.state s on
	((s.state = sc.state)));
...
CREATE VIEW devel.vw_stop_orders AS with StopLinks as (
select
	devel.stop_order.stop_type AS stop_type,
	devel.stop_order.tpsl_id AS tpsl_id,
	coalesce(devel.stop_order.client_order_id, devel.stop_order.tpsl_id) AS request_link,
	devel.stop_order.order_state AS order_state,
	devel.stop_order.order_category AS order_category,
	devel.stop_order.actual_size AS actual_size,
	row_number() OVER (PARTITION BY coalesce(devel.stop_order.client_order_id, devel.stop_order.tpsl_id),
	devel.stop_order.stop_type
ORDER BY
	devel.stop_order.tpsl_id desc ) AS rn
from
	devel.stop_order)
select
	ipos.account AS account,
	...
	sr.stop_request AS stop_request,
	if((so.request_link = so.tpsl_id), NULL, concat(sr.stop_type, so.request_link)) AS client_order_id,
	ipos.instrument_position AS instrument_position,
	ipos.instrument AS instrument,
	concat(b.symbol, '-', q.symbol) AS symbol,
	b.currency AS base_currency,
...
join devel.state s on
	((s.state = sr.state)))
left join StopLinks so on
	(((so.request_link = sr.stop_request) and (so.stop_type = sr.stop_type) and (so.rn = 1))))
left join devel.order_state os on
...
select
	...
	so.tpsl_id AS tpsl_id,
	sr.stop_request AS stop_request,
	if((so.client_order_id is null), NULL, concat(so.stop_type, so.client_order_id)) AS client_order_id,
	ipos.instrument_position AS instrument_position,
	ipos.instrument AS instrument,
	concat(b.symbol, '-', q.symbol) AS symbol,
	b.currency AS base_currency,
...
left join (
	select
		max(devel.stop_order.tpsl_id) AS tpsl_id,
		devel.stop_order.stop_type AS stop_type,
		devel.stop_order.client_order_id AS client_order_id
	from
		devel.stop_order
	where
		(devel.stop_order.client_order_id is not null)
	group by
		devel.stop_order.client_order_id,
		devel.stop_order.stop_type
union
	select
		devel.stop_order.tpsl_id AS tpsl_id,
		devel.stop_order.stop_type AS stop_type,
		devel.stop_order.tpsl_id AS client_order_id
	from
		devel.stop_order
	where
		(devel.stop_order.client_order_id is null)) oreq on
	(((oreq.client_order_id = sr.stop_request) and (oreq.stop_type = sr.stop_type))))
join devel.instrument_position ipos on
	((ipos.instrument_position = sr.instrument_position)))
join devel.instrument i on
	((i.instrument = ipos.instrument)))
...
join devel.state s on
	((s.state = sr.state)))
left join devel.stop_order so on
	(((so.tpsl_id = oreq.tpsl_id) and (so.stop_type = oreq.stop_type))))
left join devel.order_state os on
	((os.order_state = so.order_state)))
left join devel.state rs on
	((rs.status = os.status)))
