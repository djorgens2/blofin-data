...
CREATE  TABLE blofin.user ( 
	...
	update_time          DATETIME(3)  DEFAULT (now()) ON UPDATE CURRENT_TIMESTAMP NOT NULL   ,
...
CREATE  TABLE blofin.instrument_position ( 
	...
	update_time          DATETIME(3)  DEFAULT (now())  NOT NULL   ,
	close_time           DATETIME(3)  DEFAULT (now())  NOT NULL   ,
...
CREATE  TABLE blofin.request ( 
	...
	expiry_time          DATETIME(3)  DEFAULT (now())  NOT NULL   ,
	create_time          DATETIME(3)  DEFAULT (now())  NOT NULL   ,
	update_time          DATETIME(3)  DEFAULT (now())  NOT NULL   ,
...
CREATE  TABLE blofin.stop_order ( 
	...
	CONSTRAINT ak_stop_order UNIQUE ( tpsl_id, stop_type ) ,
