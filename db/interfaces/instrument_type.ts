...
export const Publish = async (props: Partial<IInstrumentType>, context = "Instrument.Type"): Promise<IPublishResult<IInstrumentType>> => {
  ...
  if (!props.instrument_type && !props.source_ref) {
  ...
  if (exists) {
    ...
    const result: TResponse = await Update<IInstrumentType>(revised, { table: `instrument_type`, keys: [[`instrument_type`]] });
    ...
    return {
      key: PrimaryKey(current, ["instrument_type"]),
      response: result,
    };
  }
...
/**
 * Searches for an instrument types based on provided criteria.
 *
