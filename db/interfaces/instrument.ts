...
import { Select, Insert, PrimaryKey, Currency } from "#db";
 ...
 * @returns A promise resolving to the publication result and primary key. Standardized via withSession.
 */
export const Publish = async (props: Partial<IInstrument>, baseContext = "Instruments"): Promise<Array<IPublishResult<IInstrument>>> => {
  ...
  if (!props.symbol) {
    return [
      {
        key: undefined,
        response: { success: false, code: 412, state: `null_query`, message: `[Error] ${context}: Symbol required`, rows: 0, context: context },
      },
    ];
  }
  ...
  return withSession(
    context,
    async (session) => {
      // Priority: Explicit prop account > Session account
      const account = props.account || session.account;

      /** 1. Check if exists */
      const exists = await Key({ account, symbol: props.symbol });

      if (exists) {
        return [
          {
            key: PrimaryKey({ instrument: exists }, [`instrument`]),
            response: { success: true, code: 201, state: `exists`, message: `Instrument already registered`, rows: 1, context: context },
          },
        ];
      }

      /** 2. Dependency Resolution (Recursive) */
      ...
      return [
        {
          key: PrimaryKey(missing, [`instrument`, `account`]),
          response: result,
        },
      ];
    },
    props.account,
  ); // Pass explicit account to withSession for Master Registry lookup
};
 ...
 *
 * @param props - Criteria to match (usually `account` &`symbol`).
...
export const Key = async (props: Partial<IInstrument>): Promise<IInstrument["instrument"] | undefined> => {
  if (props.instrument || !(props.account || props.symbol || (props.base_symbol && props.quote_symbol))) {
    return undefined;
  }

  const { success, data } = await Select<IInstrument>(props, { table: `vw_instruments` });
  return success && data?.length ? data[0].instrument : undefined;
};
