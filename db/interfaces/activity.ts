...
export const Add = async (props: Partial<IActivity>): Promise<IPublishResult<IActivity>> => {
  if (props.activity) {
    return { key: PrimaryKey(props, ["activity"]), response: { success: false, code: 200, response: `exists`, rows: 0 } };
  }
  ...
  return { key: undefined, response: { success: false, code: 404, response: `not_found`, rows: 0 } };
};
