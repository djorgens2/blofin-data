 ...
 */
export const Publish = async (props: Partial<IInstrument>, baseContext = `Instrument.Detail`): Promise<Array<IPublishResult<IInstrument>>> => {
  ...
  if (!props.instrument) {
    return [
      {
        key: undefined,
        response: { success: false, code: 414, state: `null_query`, message: `[Error] ${context}: Instrument required`, rows: 0, context },
      },
    ];
  }

  // Resolve dependencies and check for existing record in parallel
  // Note: Handling the array return from dependency Publish calls
  const [exists, itype, ctype] = await Promise.all([
    Instrument.Fetch({ instrument: props.instrument }, { table: `instrument_detail` }),
    InstrumentType.Publish({ ...props }, context),
    ContractType.Publish({ ...props }, context),
  ]);
  if (!itype?.key?.instrument_type || !ctype?.key?.contract_type) {
    return [
      {
        key: undefined,
        response: { success: false, code: 415, state: `dependency_error`, message: `[Error] ${context}: InstrumentType or ContractType key missing`, rows: 0, context },
      },
    ];
  }

  const instrument_type = itype.key.instrument_type;
  const contract_type = ctype.key.contract_type;
  
  /** 1. Insert if missing */
  if (!exists || exists.length === 0) {
    const result = await Insert({ ...props, instrument_type, contract_type }, { table: `instrument_detail`, context });
    return [{ key: PrimaryKey({ instrument: props.instrument }, ["instrument"]), response: result }];
  }

  /** 2. Diff and Update if exists */
  ...
  const result = await Update(revised, { table: `instrument_detail`, keys: [[`instrument`]], context });

  /** 3. Success Confirmation (Double-Update Pattern) */
  if (result.success) {
    const confirm = await Update(
      { instrument: current.instrument, update_time: props.update_time || new Date() },
      { table: `instrument_detail`, keys: [[`instrument`]], context },
    );
