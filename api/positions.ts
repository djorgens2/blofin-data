 ...
 */
export const Publish = async (props: Array<Partial<IPositionsAPI>>) => {
  const api: Array<IPublishResult<IPositions>> = await Promise.all(
    props.map(async (prop) => {
      ...
      if (!instrument_position) {
        return { key: undefined, response: { success: false, response: `not_found`, code: 409, rows: 0, context: "Positions.Publish" } };
      }
  ...
  if (current.length) {
    const closures = current.filter((local) => !active.some((position) => isEqual(position?.key?.instrument_position!, local.instrument_position!)));
